...
const SidebarUi = ({ sidebar }) => {
    ...
    return (
        <aside
            className={`flex flex-col min-h-screen  top-0 bg-white border-r border-gray-100 shadow-sm overflow-hidden transition-all duration-300 ease-in-out h-full w-full opacity-100`}
        >

            {/* Nav Links */}
            <nav className="flex flex-col gap-1 p-3 flex-1 min-w-[256px] ">
                {sidebar.map((s) => {
                    const Icon = s.icon;
                    return (
                        <NavLink
