...
const StoreHeader = () => {
    return (
        ...
        <section className="relative w-full min-h-[520px] md:min-h-[500px] overflow-hidden bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900">
            ...
            <div className="relative z-10 max-w-6xl mx-auto px-5 py-8 lg:px-6 lg:py-6 flex flex-col md:flex-row items-center justify-between gap-10 h-full">

                {/* Hero Text */}
                <div className="flex-1 flex flex-col gap-4 lg:gap-6 max-w-xl">
                    ...
                    <div className='flex gap-4 flex-wrap'>
                        <a href="#new-arrival" className="inline-flex items-center gap-2 bg-blue-500 hover:bg-blue-400 text-white font-semibold
                         lg:font-bold px-5 py-2 md:px-6 md:py-3 lg:px-7 lg:py-3.5 rounded-xl shadow-lg shadow-blue-500/30 transition-all duration-200 active:scale-95">
                            Shop Now →
                        </a>
                        <a href="#products" className="inline-flex items-center gap-2 text-white border border-white/30 hover:border-white/60 hover:bg-white/10 font-semibold font-semibold
                         lg:font-bold px-5 py-2 md:px-6 md:py-3 lg:px-7 lg:py-3.5  rounded-xl transition-all duration-200">
                            View All
                        </a>
                    </div>
                </div>
                ...
                <div className="flex-1 flex justify-center md:justify-end items-center">
                    <div className="relative">
                        ...
                        <img
                            src={category5}
                            alt="Home Hero"
                            className="relative w-[300px] md:w-[340px] lg:w-[400px] h-auto rounded-2xl shadow-2xl object-cover border-2 border-white/20"
                        />
...
const TrustBar = () => {
    return (
        <section className="max-w-6xl mx-auto w-full px-2 lg:px-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                    ...
                    <div key={item.title} className="flex items-center gap-3 bg-white rounded-xl border border-gray-100 shadow-sm py-2 lg:px-4 lg:py-3.5 hover:shadow-md transition-shadow">
...
const CategoryShow = ({ category }) => {
    return (
        <section className="w-full max-w-6xl mx-auto px-2 lg:px-4">
            <div className="flex items-center justify-between mb-6">
                <div>
                    <p className="text-xs font-semibold text-blue-500 uppercase tracking-widest mb-1">Explore</p>
                    <h2 className="text-2xl  font-bold text-gray-900">Shop by Category</h2>
                </div>
                <Link to="/products" className="text-sm font-semibold text-blue-600 hover:text-blue-700 flex items-center gap-1 hover:underline">
                    View all 
                </Link>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-2 lg:gap-3">
                {category.map((c) => (
                    <Link
                        key={c.id}
                        to={`/products?category=${c.name}`}
                        className="flex flex-col items-center bg-white rounded-2xl shadow-sm hover:shadow-lg transition-all duration-200 p-2 lg:p-4 group border border-gray-100 hover:border-blue-200 hover:-translate-y-1"
                    >
                        <div className="w-full h-30 rounded-xl flex items-center justify-center mb-1 lg:mb-2.5 transition-colors">
                            <img
                                src={c.icon}
                                alt={c.name}
                                className="w-full h-20 object-contain group-hover:scale-105 lg:group-hover:scale-110 transition-transform duration-300"
                            />
                        </div>
...
const SalesBanner = () => {
    return (
        <section className="max-w-6xl mx-auto w-full px-2 lg:px-4">
            <div className="rounded-2xl bg-gradient-to-r from-red-500 to-orange-400 p-4 lg:p-6 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-lg shadow-orange-200">
                <div className="flex items-center gap-4">
                    <span className="text-4xl">⚡</span>
                    <div>
                        <h3 className="text-white font-bold lg:font-extrabold text-xl md:text-2xl">Flash Sale — Up to 70% Off!</h3>
                        <p className="text-orange-100 text-sm">Limited time deals on top products. Don't miss out!</p>
                    </div>
                </div>
                <a href="#flash-sales" className="shrink-0 bg-white text-red-500 font-bold px-4 lg:px-6 py-2 lg:py-3 rounded-xl hover:bg-orange-50 transition-colors shadow text-sm">
                    Shop Flash Sale →
                </a>
            </div>
        </section>
    ...
    return (
        <section className="max-w-6xl mx-auto w-full px-2 lg:px-4">
            <div className="rounded-2xl bg-gradient-to-br from-slate-900 to-blue-950 px-8 py-10 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl">
                <div>
                    <h3 className="text-white font-extrabold text-xl md:text-2xl mb-1">News latter banner </h3>
...
const Home = () => {
    ...
    return (
        <div className="flex flex-col gap-8 lg:gap-10 bg-gray-50 ">
